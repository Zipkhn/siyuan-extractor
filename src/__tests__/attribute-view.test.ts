...
function avMap(): AvBlockMap {
    return { byNodeId: new Map([[NODE_ID, AV_JSON]]) };
}

describe("convertAttributeView — typed JSON shape (real fixture)", () => {
    it("preserves view type, names, and visible columns", () => {
        const { block } = convertAttributeView(NODE_ID, AV_JSON);
        expect(block.type).toBe("NodeAttributeView");
        if (block.type !== "NodeAttributeView") return;
        expect(block.id).toBe(NODE_ID);
        expect(block.view_type).toBe("table");
        expect(block.av_name).toBe("DB démo");
        expect(block.view_name).toBe("Table");
        expect(block.columns.map((c) => c.name)).toEqual([
            "Primary Key",
            "name",
            "Number",
            "Date",
        ]);
        expect(block.columns.map((c) => c.type)).toEqual([
            "block",
            "text",
            "number",
            "date",
    ...
    it("formats every cell type to a display string in column order", () => {
        ...
        // The fixture date is 2026-05-17 isNotTime=true. We format YYYY-MM-DD.
        expect(row.cells[3]).toMatch(/^2026-05-1[67]$/);
    });
});

describe("convertAttributeView — synthesized HTML", () => {
    it("emits a <table class=\"av-block\"> with caption, thead, tbody", () => {
        const { html } = convertAttributeView(NODE_ID, AV_JSON);
    ...
    it("escapes user content to prevent HTML injection", () => {
        ...
        const { html } = convertAttributeView(NODE_ID, malicious);
...
describe("parseAndRender — full pipeline with real Siyuan placeholder HTML", () => {
    it("emits a typed NodeAttributeView block + real <table> in HTML", () => {
        ...
        expect(avBlock.rows).toHaveLength(1);
        expect(avBlock.rows[0].cells).toContain("KOKO");

        // The sanitized HTML must contain the synthesized table, not the
        // empty Siyuan placeholder.
        expect(html).toContain('<table class="av-block">');
    ...
    it("treats unknown column types as 'unknown' without crashing", () => {
        ...
        const { block } = convertAttributeView(NODE_ID, synthetic);
        if (block.type !== "NodeAttributeView") throw new Error("wrong type");
        expect(block.columns[1].type).toBe("unknown");
        // Best-effort string: JSON of the value object, never an empty crash.
        expect(typeof block.rows[0].cells[1]).toBe("string");
    });
});
